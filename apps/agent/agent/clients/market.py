...
Two strategies are layered to handle this:
  1. Period fallback: if period="1d" returns empty (market closed / weekend),
     we retry the same request with period="5d" to get the most recent close.
  2. Tenacity retry: up to 3 attempts with exponential back-off (1 s → 2 s → 4 s)
