...
### How to get a FRED API key (free)

1. Go to **[fred.stlouisfed.org](https://fred.stlouisfed.org)**
2. Click **My Account** → **Create an Account** (top-right)
3. Fill in name, email, and password — no credit card required
4. Check your email for a verification link and confirm your account
5. Log in → click your name (top-right) → **API Keys**
6. Click **Request API Key**, enter a short description (e.g. `"Fortio FIRE tracker"`), agree to terms
7. Your key appears immediately — copy it into `.env`:
...
> **Rate limits:** 120 requests/minute on the free tier — far more than Fortio needs (max 2 calls per user question).  
> **No expiry:** Keys do not expire unless you revoke them manually.
